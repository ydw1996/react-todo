...
### 💡 개선 방향
- TypeScript 적용: 코드의 안정성과 가독성을 높이기 위해 TypeScript를 도입할 예정!
- Next.js로 확장: SEO와 서버 사이드 렌더링(SSR)을 고려해 Next.js로 마이그레이션을 시도해볼 예정
- DB 연동: MongoDB나 Firebase와 같은 데이터베이스를 연결해 데이터 영속성을 추가할 예정
